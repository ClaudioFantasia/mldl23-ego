...
class Classifier(nn.Module):
        ...
        if self.avg_modality == 'TRN':
            ...
            for i in range(self.num_clips - 1):
                relation_domain_classifier = nn.Sequential(
                    ...
                    nn.Linear(512, 2),
                    nn.Softmax()
                    # ci potrebbe stare così abbiamo delle probabilità di appartenenza ad un determinato dominio
                )
                self.relation_domain_classifier_all += [relation_domain_classifier]

        if 'PATAN' in self.domain_adapt_strategy:
            self.relation_class_classifier_all = nn.ModuleList()
            for i in range(self.num_clips - 1):
                relation_class_classifier = nn.Sequential(
                    nn.Linear(512, 512),
                    nn.ReLU(),
                    nn.Linear(512, 8),
                    nn.ReLU()
                )
                self.relation_class_classifier_all += [relation_class_classifier]
    ...
    def class_classifier_relation(self, feat_class_relation):
        # 32x4x512 --> (32x4)x8
        pred_fc_class_relation_video = None
        for i in range(len(self.relation_domain_classifier_all)):
            feat_class_relation_single = feat_class_relation[:, i, :].squeeze(1)  # 32x1x512 -> 32x512
            pred_fc_class_relation_single = self.relation_class_classifier_all[i](feat_class_relation_single)
            # feat_fc_domain_relation_single = GradReverse.apply(feat_relation_single,beta)  # the same beta for all relations (for now)
            # pred_fc_domain_relation_single = self.relation_domain_classifier_all[i](feat_fc_domain_relation_single)
...
class RelationModuleMultiScale(torch.nn.Module):
    ...
    def __init__(self, img_feature_dim, num_bottleneck, num_frames):
        ...
        for scale in self.scales:
            relations_scale = self.return_relationset(num_frames,
                                                      scale)  # num_frames/clips = 5, quindi avrò diversi (0,1) (0,2) (0,3) (0,4) (1,2) (1,3) se scale = 2
